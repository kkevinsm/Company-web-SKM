            ...
            <div class="container mx-auto px-6">
                <div class="flex flex-col md:flex-row items-center gap-12">
                    ...
                    <div x-data="{ 
                        ...
                        reelsUrl: '',
                        isVisible: false,
                        fadeAudio(video, start, end, duration) {
                            const steps = 20;
                            ...
                            const fadeInterval = setInterval(() => {
                                current = Math.min(end, current + stepValue);
                        ...
                        init() {
                            ...
                            fetch('/videos/reels/latest_url.txt')
                                .then(response => response.text())
                                .then(url => this.reelsUrl = url.trim())
                            ...
                            const observer = new IntersectionObserver((entries) => {
                                entries.forEach(entry => {
                                    ...
                                    if (entry.isIntersecting) {
                                        video.play().then(() => {
                                            if (!video.muted) {
                                                this.fadeAudio(video, 0, 1, 1000); // Fade in over 1 second
                                            }
                                        });
                                    } else {
                                        video.pause();
                            ...
                            // Handle unmute with fade
                            video.addEventListener('volumechange', () => {
                                if (!video.muted && video.volume === 0) {
                                    this.fadeAudio(video, 0, 1, 1000);
                                }
                            });
                        }
                    }" class="md:w-2/5 flex justify-center relative z-20">
                        <div class="relative mx-auto border-gray-800 bg-gray-800 border-[14px] rounded-[2.5rem] h-[500px] w-[250px] shadow-xl transition-transform duration-700 ease-in-out hover:scale-105 hover:shadow-2xl group">
                            ...
                            <div class="rounded-[2rem] overflow-hidden w-full h-full bg-black">
                                <video
                                    class="w-full h-full object-cover transform transition-transform duration-700 ease-in-out"
                                    src="{{ asset('videos/reels/latest_reels.mp4') }}"
                                    loop
                                    playsinline
                                    x-ref="video"
                                ...
                                <div x-show="showControls" 
                                     @mouseover="showControls = true"
                                     @mouseout="showControls = false"
                                     class="absolute inset-x-0 bottom-0 flex flex-col gap-3 opacity-0 transition-opacity duration-800"
                                    ...
                                    <div class="flex items-center justify-end gap-3 px-4 pb-2">
                                        <button @click="$refs.video.paused ? $refs.video.play() : $refs.video.pause()"
                                                class="w-8 h-8 flex items-center justify-center rounded-full bg-white/20 backdrop-blur-sm text-white hover:bg-white/30 transition-all duration-800">
                                            <svg x-show="$refs.video.paused" class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <button @click="$refs.video.muted = !$refs.video.muted"
                                                class="w-8 h-8 flex items-center justify-center rounded-full bg-white/20 backdrop-blur-sm text-white hover:bg-white/30 transition-all duration-800">
                                            <svg x-show="$refs.video.muted" class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
